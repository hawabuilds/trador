...
/**
 * The structural claim behind the strong issuers.
 *
 * "xStocks" means something precise here: one key mints the whole range. If a
 * second authority ever appears under that issuer, either an issuer rotated a
 * key or something got in that does not belong, and both need a human before
 * the feed trusts it.
 */
test("issuers verified by issuer key have exactly one authority each", () => {
  for (const issuer of Object.keys(ISSUERS) as StockIssuer[]) {
    if (ISSUERS[issuer].verification !== "issuer-key") continue;
    ...
    assert.equal(
      authorities.size,
      1,
      `${issuer} has ${authorities.size} authorities: ${[...authorities].join(", ")}`,
    );
  }
});
...
test("known lookalikes are not in the registry", () => {
  const mustBeAbsent: [string, string, string][] = [
    ...
    [
      "NKEda5nHhNGgjrE9nDdMvaEmkmJ96qqxzBVZEcKmjSg",
      "NKE (Backpack Securities)",
      "a genuine tokenized equity from a third issuer whose mints each carry " +
        "their own authority. Admitting it requires a maintained per-mint " +
        "allowlist, which is a deliberate product decision (INCLUDE_BACKPACK=1) " +
        "and not the default.",
    ],
  ];
